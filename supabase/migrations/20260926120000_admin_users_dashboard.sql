-- Administrator-only account and legitimate purchase-interest reporting.
-- auth.users remains the authoritative identity source; no auth secrets are returned.
...
begin
  ...
  with role_rollup as (
    select ur.user_id,
      case
        when bool_or(ur.role in ('admin', 'owner')) then 'admin'
    ...
    where o.user_id is not null and o.is_test = false
    group by o.user_id
  ), restock_rollup as (
    select lower(r.email) as email, count(*)::integer as restock_count,
      max(r.created_at) as latest_restock_at
    from public.restock_requests r
    group by lower(r.email)
  ), base as (
    select u.id,
      u.email,
      coalesce(rr.account_role, 'customer') as role,
      u.created_at,
      ...
      ord.latest_order_at,
      null::integer as cart_item_count,
      coalesce(res.restock_count, 0) as restock_count,
      greatest(ord.latest_order_at, res.latest_restock_at) as latest_signal_at,
      case
        when coalesce(ord.order_count, 0) > 0 then 'A plasat comandă'
        when coalesce(res.restock_count, 0) > 0 then 'Cerere revenire în stoc'
        else 'Cont înregistrat'
      end as source_label
    from auth.users u
...
revoke all on function public.admin_users_dashboard(text,text,text,text,timestamptz,timestamptz,text,integer,integer)
  from public, anon;
grant execute on function public.admin_users_dashboard(text,text,text,text,timestamptz,timestamptz,text,integer,integer)
  to authenticated;
...
begin
  ...
  with events as (
    select 'account-' || u.id::text as id, u.email, 'account'::text as source,
      'Cont înregistrat'::text as source_label,
      null::text as summary, u.created_at as activity_at, null::text as order_status
    from auth.users u
    where u.deleted_at is null and u.email is not null
    union all
...
revoke all on function public.admin_interest_dashboard(text,text,timestamptz,timestamptz,integer,integer)
  from public, anon;
grant execute on function public.admin_interest_dashboard(text,text,timestamptz,timestamptz,integer,integer)
  to authenticated;
...
begin
  ...
  payload := public.admin_users_dashboard(
    _search, _role, _source, _status, _date_from, _date_to, _sort, 1, 100
  );
  ...
  select auth.uid(), u.email, 'accounts.csv_exported', 'admin_users_dashboard',
    jsonb_build_object(
      ...
      'filters', jsonb_build_object(
        'search', _search, 'role', _role, 'source', _source, 'status', _status,
  ...
  return jsonb_build_object(
    'rows', payload->'rows',
    'rowCount', exported_rows,
    'truncated', ((payload->>'filteredCount')::integer > exported_rows)
  );
end;
$$;

revoke all on function public.admin_users_export(text,text,text,text,timestamptz,timestamptz,text)
  from public, anon;
grant execute on function public.admin_users_export(text,text,text,text,timestamptz,timestamptz,text)
  to authenticated;

-- Retention periods must be confirmed by the company/legal reviewer before automation.
...
-- Supply a clear privacy draft when the project still has the original empty placeholder.
-- Existing company-authored wording is never overwritten by this migration.
...
where key = 'privacy'
  and nullif(trim(coalesce(value->>'body', '')), '') is null;
